<script lang="ts">
  ...
  const keyMan = KeyManager.subscribe({
    onArrowLeft: () => {
      ...
      if (prevTweet) replace(`/user/${params.userId}/tweets/${prevTweet.id}`);
    ...
    onArrowRight: () => {
      ...
      if (nextTweet) replace(`/user/${params.userId}/tweets/${nextTweet.id}`);
  ...
  onMount(async () => {
    tweets = await new DataService().getUserTweets(params.userId);
    if (!params.tweetId) {
      replace(`/user/${params.userId}/tweets/${tweets[0].id}`);
    }
